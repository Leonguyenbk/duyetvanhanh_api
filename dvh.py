...
def nap_chu_su_dung(duong_dan_json):
    """
    Đọc file JSON cấu trúc {"ChuSoHuu": {...}} và trả về object ChuSoHuu bên trong.
    Toàn bộ thông tin chủ lấy từ file, không cần nhập gì thêm.
    """
    if not os.path.isfile(duong_dan_json):
        raise RuntimeError(f"Không thấy file: {duong_dan_json}")
    with open(duong_dan_json, "r", encoding="utf-8") as f:
        data = json.load(f)

    if not isinstance(data, dict) or "ChuSoHuu" not in data or not isinstance(data["ChuSoHuu"], dict):
        raise RuntimeError(
            f"File {os.path.basename(duong_dan_json)} sai cấu trúc: "
            f"phải là {{\"ChuSoHuu\": {{...}}}} với key ChuSoHuu ở ngoài cùng."
        )

    tpl = data["ChuSoHuu"]
    # Phải có ít nhất 1 chủ trong bất kỳ nhóm nào
    cac_nhom = ("ToChucs", "CaNhans", "VoChongs", "HoGiaDinhs", "CongDongs", "NhomNguois")
    if not any(isinstance(tpl.get(n), list) and tpl.get(n) for n in cac_nhom):
        raise RuntimeError(
            f"ChuSoHuu trong {os.path.basename(duong_dan_json)} trống — không có chủ nào "
            f"trong {', '.join(cac_nhom)}."
        )
    return tpl


def ten_chu_trong_template(tpl):
    """Lấy chuỗi tên các chủ trong object ChuSoHuu để hiển thị."""
    cac_ten = []
    for loai in ("ToChucs", "CaNhans", "HoGiaDinhs", "CongDongs", "NhomNguois", "VoChongs"):
        for item in (tpl.get(loai) or []):
            if isinstance(item, dict):
...
class App:
    def __init__(self, root):
        ...
        # File JSON chứa thông tin chủ (thay nguyên ChuSoHuu trong payload)
        ttk.Label(frm, text="File JSON chủ sử dụng:").grid(row=5, column=0, sticky="w")
        self.var_json_chu = tk.StringVar()
        ttk.Entry(frm, textvariable=self.var_json_chu, width=60).grid(
            row=5, column=1, columnspan=2, sticky="we", padx=5, pady=3
        )
        ttk.Button(frm, text="Chọn JSON...", command=self.chon_json_chu).grid(row=5, column=3, sticky="w")
        ttk.Label(frm, text="(để trống = giữ nguyên chủ trong payload)").grid(
    ...
    def chay(self):
        ...
        # Chủ sử dụng: chọn file JSON thì thay nguyên ChuSoHuu, để trống thì giữ nguyên
        chu_template = None
        json_path = self.var_json_chu.get().strip()
        if json_path:
            try:
                chu_template = nap_chu_su_dung(json_path)
            except Exception as e:
        ...
        if not messagebox.askyesno(
            ...
            f"Chủ sử dụng: {ten_chu_trong_template(chu_template) + ' (từ ' + os.path.basename(json_path) + ')' if chu_template else '(giữ nguyên payload)'}\n"
